module ocean_velocity_mod
  ...
  ! <DATA NAME="nofriction" TYPE="logical" DEFAULT=".false.">
  !   If true, ignores raleigh_timescale and sets ray_coeff = 0.0
  ! </DATA>  
  ...
  namelist /ocean_velocity_nml/ taux0, tauy0,&
                                ...
                                rayleigh_timescale, nofriction,diag_freq, wind_type 
...
contains 
  ...
  subroutine ocean_velocity_init (Domain_in, Grid_in, Time_in, nonlinear_in, Velocity)
   ...
   !--- regrid taux to u-cell and tauy to v-cell points.
    ...
    id_ke   = register_diag_field ('ocean_model', 'enk1',  &
                     Time%Model_time, 'ke_per_unitarea', 'J/m^2', missing_value=-999.0, range=(/0.0,999.0/) )
    id_pe   = register_diag_field ('ocean_model', 'enp1',  &
                     Time%Model_time, 'pe_per_unitarea', 'J/m^2', missing_value=-1000.0, range=(/-999.0,999.0/) )
  ...
  subroutine update_ocean_velocity(Velocity, Freesurf)
    ...
    logical :: used
    real    :: twodt, temp, curl, enk, enp,enk1,enp1
    real    :: tmp(isc:iec, jsc:jec)
    real    :: tmp1(isc:iec, jsc:jec)
       ...
       if(mod(Time%itt,diag_freq) == 0 ) then
          do j = jsc, jec
             do i = isc, iec
                ...
                tmp1(i,j) = Grid%ht(i,j)*Grid%area(i,j)*(Velocity%u(i,j,taum1)**2 + Velocity%u(i+1,j,taum1)**2 &
                     + Velocity%v(i,j,taum1)**2 + Velocity%v(i,j+1,taum1)**2)
             enddo
          enddo
          enk = 0.125*rho0*mpp_global_sum(Domain, tmp)/Grid%totarea
          enk1 = 0.125*rho0*mpp_global_sum(Domain, tmp1)/Grid%totarea
          do j = jsc, jec
             do i = isc, iec
                tmp(i,j) = Grid%area(i,j)*Freesurf%eta(i,j,tau)**2
                tmp1(i,j) = Grid%area(i,j)*Freesurf%eta(i,j,taum1)**2
             enddo
          enddo
          enp = 0.5*GRAV*mpp_global_sum(Domain, tmp)/Grid%totarea
          enp1 = 0.5*GRAV*mpp_global_sum(Domain, tmp1)/Grid%totarea
...
! Send out ke and pe
    if (id_ke > 0) used = send_data (id_ke, enk1, Time%Model_time)
    if (id_pe > 0) used = send_data (id_pe, enp1, Time%Model_time)
