...
! Not sure how this works for periodic condidtions and halo > 1.
      ...
      if (nb_left .eq. MPI_PROC_NULL) u(0,:,tau) = 0.0
      if (nb_right .eq. MPI_PROC_NULL) u(ni+1,:,tau) = 0.0
      if (nb_bot .eq. MPI_PROC_NULL) u(:,0,tau) = 0.0
      if (nb_top .eq. MPI_PROC_NULL) u(:,nj+1,tau) = 0.0
      
!     Initial conditions
      do j = jsc,jec
        do i = isc,iec
          u(i,j,tau) = sin(pi*local_domain%x_local(i))*sin(pi*local_domain%y_local(j))
        enddo
      enddo
