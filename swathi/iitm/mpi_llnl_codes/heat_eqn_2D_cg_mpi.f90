!
! Solve this with conjugate gradient
!
      module heat_types
      type domain2D
...
!
      ...
      real :: eps = 1.0e-10, delta = 1.0e-10
      integer :: nit_max = 100
...
!
      ...
      call conjg_grad_5pt(local_domain,isc,iec,jsc,jec,isd,ied,jsd,jed,ap,aw,ae,as,an,eps,delta,nit_max,usol,rhs)
      u(:,:,taup1) = usol(:,:)
      return
      end
!  
! First let us do conjugate gradient; No preconditioning for now
! Kincaid-Cheney pg 211

      subroutine conjg_grad_5pt(local_domain,isc,iec,jsc,jec,isd,ied,jsd,jed,ap,aw,ae,as,an,eps,delta,nit_max,x,b)
      use mpi
      use heat_types
      type (domain2D) :: local_domain
      ...
      real :: eps, delta, c, t, d,vdotv,vdotz
...
!
      ...
      call vec_prod(isd,ied,jsd,jed,isc,iec,jsc,jec,r,r,c)
      call MPI_BARRIER(MPI_COMM_WORLD,ierr)
      call MPI_ALLREDUCE(c,c,1,MPI_REAL,MPI_SUM,MPI_COMM_WORLD,ierr)
!
      k = 0
      do while( k .le. nit_max )
        ...
        if ( sqrt(vdotv) .le. delta) then
          ...
          x= x + t*v
          r=r-t*z
          call vec_prod(isd,ied,jsd,jed,isc,iec,jsc,jec,r,r,d)
          call MPI_BARRIER(MPI_COMM_WORLD,ierr)
          call MPI_ALLREDUCE(d,d,1,MPI_REAL,MPI_SUM,MPI_COMM_WORLD,ierr)
          if (d**2 .le. eps) then
          write(*,*)'Converged in',k,'iterations'
            go to 100
          else
