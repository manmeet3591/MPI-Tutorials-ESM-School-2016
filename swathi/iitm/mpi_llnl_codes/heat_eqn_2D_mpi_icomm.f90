!
! We use isend and irecv instead of sendrecv
      module heat_types
      type domain2D
...
! ISENDS
      ...
      if ( nb_top /= MPI_PROC_NULL) call MPI_ISEND(sb_buf,ni_local_tot,MPI_REAL,nb_top, &
                      tag,MPI_COMM_WORLD, request(1), ierr)
      if ( nb_bot /= MPI_PROC_NULL) call MPI_ISEND(st_buf,ni_local_tot,MPI_REAL,nb_bot, & 
                      tag,MPI_COMM_WORLD, request(2), ierr)
      if ( nb_left /= MPI_PROC_NULL) call MPI_ISEND(sl_buf,nj_local_tot,MPI_REAL,nb_left, &
                      tag,MPI_COMM_WORLD, request(3), ierr)
      if ( nb_right /= MPI_PROC_NULL)call MPI_ISEND(sr_buf,nj_local_tot,MPI_REAL,nb_right, & 
                      tag,MPI_COMM_WORLD, request(4), ierr)
! Receives
      if (nb_top /=  MPI_PROC_NULL) call MPI_RECV(rt_buf,ni_local_tot,MPI_REAL,nb_top, &
                      tag,MPI_COMM_WORLD, status, ierr)
      if (nb_bot /=  MPI_PROC_NULL) call MPI_RECV(rb_buf,ni_local_tot,MPI_REAL,nb_bot, &
                      tag,MPI_COMM_WORLD, status, ierr)
      if (nb_left /=  MPI_PROC_NULL) call MPI_RECV(rl_buf,nj_local_tot,MPI_REAL,nb_left, &
                      tag,MPI_COMM_WORLD, status, ierr)
      if (nb_right /=  MPI_PROC_NULL) call MPI_RECV(rr_buf,nj_local_tot,MPI_REAL,nb_right, &
                      tag,MPI_COMM_WORLD, status, ierr)
...
! Now put them back in the u array
      ...
      if (nb_left /=  MPI_PROC_NULL) u(isc-xhalo:isc-1,jsc:jec) =  rl_buf(1:xhalo,1:nj_local)
      if (nb_right /=  MPI_PROC_NULL) u(iec+1:iec+xhalo,jsc:jec) =  rr_buf(1:xhalo,1:nj_local)
      if (nb_bot /=  MPI_PROC_NULL)  u(isc:iec,jsc-xhalo:jsc-1) =  rb_buf(1:ni_local,1:yhalo)
      if (nb_top /=  MPI_PROC_NULL) u(isc:iec,jec+1:jec+yhalo) =  rt_buf(1:ni_local,1:yhalo)
!        
...
!
      ...
      real :: delx,dely,del_t
      real :: u(isd:ied,jsd:jed,0:1)
      do j = jsc,jec
        do i = isc,iec
          u(i,j,taup1) = u(i,j,tau)*(1-2*del_t*(delx**2 + dely**2)/(delx**2*dely**2)) + &
                           del_t*((u(i-1,j,tau) + u(i+1,j,tau))/delx**2 + &
                                  (u(i,j-1,tau) + u(i,j+1,tau))/dely**2)
        enddo
      enddo
      return
      end
