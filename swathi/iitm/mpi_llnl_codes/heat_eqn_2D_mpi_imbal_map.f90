! Swathi...10/7/2015...Will allow node imbalance
      module heat_types
      type domain2D
...
!
      ...
      if ( mod(ni,domain_layout(1)) .ne. 0 ) then
        write(*,*) 'Load imbalance in the x-dir,',ni,domain_layout(1)
      endif

      if ( mod(nj,domain_layout(2)) .ne. 0 ) then
        write(*,*) 'Load imbalance in the y-dir ,',nj,domain_layout(2)
      endif
...
! ni and nj will be powers of 2 raised to domain layout
!
      if (is_block_index .eq. 1) then 
        isc = 1
        iec = 2
      else
        isc = 2**(is_block_index-1) + 1
        iec = 2**is_block_index
      endif
      if (js_block_index .eq. 1) then 
        jsc = 1
        jec = 2
      else 
        jsc = 2**(js_block_index -1) + 1
        jec = 2**js_block_index
      endif
      if (is_block_index .eq. domain_layout(1)) then
        iec = max(iec,ni)
      endif
      if (js_block_index .eq. domain_layout(2)) then
        jec = max(jec,nj)
      endif  
      isd = isc-xhalo;ied = iec+xhalo 
      jsd = jsc-yhalo;jed = jec+yhalo
